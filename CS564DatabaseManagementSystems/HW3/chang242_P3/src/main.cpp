...
// If the relation size is changed 
// then the second parameter at checkPassFail may need to be changed to number of record 
// that are expected to be found during the scan, else tests will erroneously be reported to have failed.
const int	relationSize = 5000 * 70;
...

	{
		...
		try
		{
			...
			while(1)
			{
				...
				// std::cout << offsetof(RECORD, i) << "offsetof (RECORD, i)\n";
				// std::cout << (record + offsetof (RECORD, i)) << "add\n";
...
void createRelationRandom()
{
	...
	catch(FileNotFoundException e)
	{
  	...
  	// initialize all of record1.s to keep purify happy
  	// this is the structure for record1, type of record1 is RECORD
	// memset sets record1.s to " "
  	memset(record1.s, ' ', sizeof(record1.s));
	PageId new_page_number;
  	Page new_page = file1->allocatePage(new_page_number);

  	// insert records in random order, relationSize is 5000
	// so we have a vertor of 5000 ints from 0 to 4999
	...
	int i = 0;
  	while(i < relationSize)
  	{
		...
		val = intvec[pos];
		// instead of printing on console, it stores "val string record" on record1.s
		sprintf(record1.s, "%05d string record", val);
		record1.i = val;
		record1.d = val;

		// tranforms the RECORD strut into a string, and we consider this string as a data format
		std::string new_data(reinterpret_cast<char*>(&record1), sizeof(RECORD));
		...
		while (1)
		{
			try
			{	
				// insert our new data into the new page of our page file
    			new_page.insertRecord(new_data);
				break;
			}
			catch(InsufficientSpaceException e)
			{	
				// when the current page is not enough for inserting a new data, we create a new page  
				file1->writePage(new_page_number, new_page);
				new_page = file1->allocatePage(new_page_number);
			}
		}

		// exchange the value of intvec[relationSize - 1 - i] and the value of intvec[pos],
		// so we don't insert a value twice to the pagefile
...
/**
 * This method returns ture when a BadScanrangeException is thrown under a certain condition
 */
bool badScanrangeExceptionCheck(BTreeIndex * index, int lowVal, Operator lowOp, int highVal, Operator highOp) {
	try {
		intScan(index, lowVal, lowOp, highVal, highOp);
...
/**
 * This method returns ture when a BadOpcodesException is thrown under a certain condition
 */
bool badOpcodesExceptionCheck(BTreeIndex * index, int lowVal, Operator lowOp, int highVal, Operator highOp) {
	try {
		intScan(index, lowVal, lowOp, highVal, highOp);
...
/**
 * This method returns ture when the number of NonLeafNodes is equal to nonLeaf and the number of LeafNodes
 * is equal to leaf 
 */
bool numNodesCheck(BTreeIndex * index, int nonLeaf, int leaf) {
	if (index->getNumNonLeafNodes() == nonLeaf && index->getNumLeafNodes() == leaf) {
		return true;
	}
...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	// custom test: end key
	checkPassFail(intScan(&index,4998,GTE,4999,LTE), 2)

	// custom test: badOpcodesException
	if (badOpcodesExceptionCheck(&index,200,LT,300,GT)) {
		std::cout << "badOpcodesException test passed\n\n";
	} 

	// custom test: badScanrangeException
	if (badScanrangeExceptionCheck(&index,400,GT,300,LT)) {
		std::cout << "badScanrangeException test passed\n\n";
	} 

	// custom test: check number of nodes
	if (numNodesCheck(&index, 3, 1026)) {
		std::cout << "number of nodes test passed\n\n";
	} else {
		std::cout << "make sure that raltionSize is 350000, and createRandom would not pass this test\n\n";
	}
}

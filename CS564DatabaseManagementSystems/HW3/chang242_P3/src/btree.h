...
class BTreeIndex {
 ...
 private:
  ...
  /**
   * Number of pages used in this BPtree for nonleaf nodes
   */
  int     numNonLeafNodes;

  /**
   * Number of pages used in this BPtree for leaf nodes
   */
  int     numLeafNodes;
 ...
 public:
   ...
   * If nonleadnode is full, we split nonleafnode, and add a new key to the parent nonleadnode.
   * 
   * @param curPage current page is equal to current node, and it is a node that we traverse thourgh or update
   ...
   * @param PageKeyPair a pair that is used to update nonleafnode, when a children node of a parent node is splited
   */
  const void insertNode(Page *curPage, PageId curPageNum, bool nodeIsLeaf, const RIDKeyPair<int> dataEntry, PageKeyPair<int> *&newchildEntry);

  /**
   * Find the childNode of the curNode based on key
   * 
   * @param curNode current node is a parent
   * @param nextNodeNum the page number of a children node
   * @param key is used to locate a children node among all the children of current node
   */

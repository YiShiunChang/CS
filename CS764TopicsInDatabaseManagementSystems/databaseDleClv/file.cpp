...
Page PageFile::allocatePage(PageId &new_page_number) {
  // when a PageFile is created, its header = {1, 0, 0, 0}
  FileHeader header = readHeader();
  Page new_page;
  Page existing_page;
  if (header.num_free_pages > 0) {
    // std::cout << "num_free_pages > 0 start" << std::endl;
    ...
    if (header.first_used_page == Page::INVALID_NUMBER || header.first_used_page > new_page.page_number()) {
      ...
      // INVALID_NUMBER = 0, number of page indicating that it's invalid.
      // page.page_number() returns current_page_number of the page
      ...
      for (FileIterator iter = begin(); iter != end(); ++iter) {
        ...
        if (next_page_number > new_page.page_number() || next_page_number == Page::INVALID_NUMBER) {
          existing_page = *iter;
          break;
        }
      }
    ...
    assert((header.num_free_pages == 0) == (header.first_free_page == Page::INVALID_NUMBER));
  } else {
    // std::cout << "num_free_pages <= 0 start" << std::endl;
    ...
    if (header.first_used_page == Page::INVALID_NUMBER) {
      ...
      // If we have pages allocated, we need to add the new page to the tail of the linked list.
      for (FileIterator iter = begin(); iter != end(); ++iter) {

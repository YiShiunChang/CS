...
/**
 * @brief Class which represents a file in the filesystem containing database pages.
 *
 * The File class wraps a stream to an underlying file on disk. Files contain fixed-sized pages, 
 * and they never deallocate space (though they do reuse deleted pages if possible).  
 * If multiple File objects refer to the same underlying file, they share the stream in memory.
...
class File {
 public:
   ...
   * @throws FileExistsException If the underlying file exists and create_new is true.
   * @throws FileNotFoundException If the underlying file doesn't exist and create_new is false.
   */
   ...
   * @return The page.
   * @throws InvalidPageException If the page doesn't exist in the file or is not currently used.
   */
 ...
 protected:
  ...
  // A friend class can access private and protected members of another class where it is declared 
  // as friend.
...
public:
   ...
   * Opens the file named fileName and returns the corresponding File object.
	 * It first checks if the file is already open. If so, then the new File object created uses the
   * same input-output stream to read to or write fom that already open file. 
   * Reference count (open_counts_ static variable inside the File object) is incremented whenever 
   * an already open file is opened again. 
   * Otherwise the UNIX file is actually opened. The fileName and the stream associated with this 
   * File object are inserted into the open_streams_ map.
   *
   * @param filename  Name of the file.
 ...
 private:
  ...
  /**
   * Reads a page from the file. If <allow_free> is not set, an exception will be thrown if the 
   * page read from disk is not currently in use.
   * No bounds checking is performed; the underlying file stream will throw an exception if the 
   * page is past the end of the file.
   *
   * @param page_number Number of page to read.
   * @param allow_free Whether to allow reading a free (unused) page.
   * @return  The page.
   * @throws InvalidPageException If the page is free (unused) and allow_free is false.
   */
   ...
   * This does not ensure that the number in the header equals the position on disk. 
   * No bounds checking is performed.
   *
   * @param page_number Number of page whose contents to replace.
  ...
  /**
   * Reads only the header of the given page from disk (not the record data  or slot table).  
   * No bounds checking is performed.
   *
...
public:
  ...
  /**
   * Opens the file named fileName and returns the corresponding File object.
	 * It first checks if the file is already open. If so, then the new File object created uses 
   * the same input-output stream to read to or write fom that already open file. 
   * Reference count (open_counts_ static variable inside the File object) is incremented whenever 
   * an already open file is opened again. 
   * Otherwise the UNIX file is actually opened. The fileName and the stream associated with this 
   * File object are inserted into theopen_streams_ map.
   *
   * @param filename  Name of the file.
   * @throws  FileNotFoundException   If the requested file doesn't exist.
   */
